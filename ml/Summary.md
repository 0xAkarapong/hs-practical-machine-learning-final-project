...
4. **Baseline** — `DummyRegressor(strategy="mean")` as a sanity-check reference. `DummyRegressor` ignores `X` entirely — it memorizes `mean(y_train)` and emits that constant for every row, so the baseline **never uses the features** (it is fed `X_train` only because `fit()` requires an `X` argument syntactically). Evaluated on the held-out test set with the same `evaluate_model` as the real models, so the comparison is in the same log + THB metric space. It predicts `mean(y_train)`, not `mean(y_test)`, so R² lands **≈ 0 (not exactly 0)** — the leak-free version of an R²=0 reference (a baseline that hit exactly 0 would have to peek at the test mean). Measured R² ≈ −0.0001; both GBDT (+0.39) and XGBoost (+0.47) beat it.
