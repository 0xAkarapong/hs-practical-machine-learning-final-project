...
def load_raw() -> pd.DataFrame:
    ...
    print(
        f"rows {df.shape[0]} | columns {list(df.columns)} | Section cats {n_sections}"
    )
    return df
...
def missingness(df: pd.DataFrame) -> pd.DataFrame:
    ...
    fig.tight_layout()
    fig.savefig(
        FIGURES_DIR / "missingness.png", dpi=150, facecolor=SURFACE, bbox_inches="tight"
    )
    plt.close(fig)
...
def price_profile(df: pd.DataFrame) -> pd.DataFrame:
    ...
    fig.tight_layout()
    fig.savefig(
        FIGURES_DIR / "price_dist.png", dpi=150, facecolor=SURFACE, bbox_inches="tight"
    )
    plt.close(fig)
    ...
    fig.tight_layout()
    fig.savefig(
        FIGURES_DIR / "price_by_section.png",
        dpi=150,
        facecolor=SURFACE,
        bbox_inches="tight",
    )
...
def categorical_profile(df: pd.DataFrame) -> None:
    ...
    fig.tight_layout()
    fig.savefig(
        FIGURES_DIR / "section_counts.png",
        dpi=150,
        facecolor=SURFACE,
        bbox_inches="tight",
    )
    fig.savefig(
        FIGURES_DIR / "location_counts.png",
        dpi=150,
        facecolor=SURFACE,
        bbox_inches="tight",
    )
...
def price_vs_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    order = (
        sub.groupby("Shop Location")["Price"]
        .median()
    ...
    fig.tight_layout()
    fig.savefig(
        FIGURES_DIR / "price_by_location.png",
        dpi=150,
        facecolor=SURFACE,
        bbox_inches="tight",
    )
