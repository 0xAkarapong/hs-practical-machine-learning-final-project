...
ORIGINAL_COLUMNS = ["Id", "Section", "Name", "Price", "Total Sold", "Total Reviews", "Shop Location"]
...
def flag_anomalies(df: pd.DataFrame) -> pd.DataFrame:
    ...
    print(f"B per-section |z|>{ROBUST_Z_THRESHOLD}: {df['anom_robust'].sum()} ({df['anom_robust'].mean() * 100:.1f}%)")
...
def drop_anomalies(flagged: pd.DataFrame) -> pd.DataFrame:
    ...
    print(f"before {n_before} -> after {len(clean)} (dropped {n_before - len(clean)}, {flagged[DROP_FLAG].mean() * 100:.1f}%)")
    return clean
...
def plot_anomalies(df: pd.DataFrame, clean: pd.DataFrame, figures_dir: Path) -> None:
    ...
    sns.boxplot(data=df, y="Section", x="Price", order=order, ax=ax, fliersize=0, color=SERIES_BLUE, width=0.6)
    anom = df[df["anom_both"]]
    sns.stripplot(data=anom, y="Section", x="Price", order=order, ax=ax, color=SERIES_RED, size=2, alpha=0.6)
    ax.set_xscale("log")
    ax.set_title("Price by Section (log x) — red = high-confidence anomalies (IQR ∩ robust-z)")
    fig.tight_layout()
    fig.savefig(figures_dir / "anomaly_price_by_section.png", dpi=150, facecolor=SURFACE, bbox_inches="tight")
    plt.close(fig)
    print(f"Saved:  anomaly_price_by_section.png → notebooks/figures/")

    # Before/after price histograms (raw + log1p).
    fig, axes = plt.subplots(1, 2, figsize=(11, 4))
    sns.histplot(df["Price"], bins=80, ax=axes[0], color=SERIES_BLUE, label="before")
    sns.histplot(clean["Price"], bins=80, ax=axes[0], color=SERIES_RED, label="after", alpha=0.6)
    axes[0].set_title("Price (raw, ฿)")
    axes[0].legend()
    sns.histplot(np.log1p(df["Price"]), bins=80, ax=axes[1], color=SERIES_BLUE, label="before")
    sns.histplot(np.log1p(clean["Price"]), bins=80, ax=axes[1], color=SERIES_RED, label="after", alpha=0.6)
    axes[1].set_title("Price (log1p)")
    axes[1].legend()
    fig.tight_layout()
    fig.savefig(figures_dir / "anomaly_before_after.png", dpi=150, facecolor=SURFACE, bbox_inches="tight")
    plt.close(fig)
    print(f"Saved:  anomaly_before_after.png → notebooks/figures/")
